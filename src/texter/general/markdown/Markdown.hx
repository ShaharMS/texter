...
class Markdown
{
	...
	static function get_isParagraphStart():Bool
		return isParagraphStart = !isParagraphStart;
	...
	public static function interpret(markdownText:String, onComplete:(String, Array<MarkdownEffect>) -> Void)
	{
		...
		// fix for nested bold
		while (lineTexts.contains("__"))
			lineTexts = lineTexts.replacefirst("__", "**");
		...
		for (rule in markdownRules)
		{
			while (rule.match(current))
			{
				if (rule == patterns.italicEReg || rule == patterns.mathEReg || rule == patterns.codeEReg)
				{
					...
					effects.push(if (rule == patterns.italicEReg) Italic(info.pos,
						info.pos + info.len) else if (rule == patterns.codeEReg) Code(info.pos, info.pos + info.len) else Math(info.pos, info.pos + info.len));
				}
				...
				else if (rule == patterns.listItemEReg)
				{
					if (!~/[0-9]/g.match(rule.matched(1)))
						current = rule.replace(current, "$1· $3")
					else
						current = rule.replace(current, "$1$2. $3");
					var info = rule.matchedPos();
