...
class MathTextField extends TextFieldCompatibility
{
	...
	public function render(e:Event)
	{
		...
		var mathProps:Array<MathAttribute> = MathLexer.resetAttributesOrder(MathLexer.splitBlocks(MathLexer.getMathAttributes(text)));
		...
		for (element in mathProps)
		{
			...
			switch element
			{
				...
				case Closure(index, letter, content):
					var t = new DrawableTextField();
					t.defaultTextFormat = new TextFormat(null,
						smallClosure ? Std.int(textField.defaultTextFormat.size / 2) : textField.defaultTextFormat.size);
		...
		trace("gets to line 93");
		var xPos = 0.;
		trace(textFields);
		for (t in textFields)
		{
			addChild(t);
			if (defaultTextFormat.bold)
				t.defaultTextFormat = new TextFormat("assets/texter/MathTextField/math-bold.ttf", 40, 0x000000);
			else
				t.defaultTextFormat = new TextFormat("assets/texter/MathTextField/math-regular.ttf", 40, 0x000000);
