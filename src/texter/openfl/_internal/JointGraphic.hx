...
class JointGraphic
{
	...
	public function new(d:DynamicTextField)
	{
		this.d = d;
	}
	...
	function set_defaultGraphic(g:BitmapData):BitmapData
	{
		...
		{
			if (defaultEdgeGraphic != null)
				break;
			if (i != null)
				continue;
			if (i == top)
				ref = d.joints.middleTop;
			if (i == left)
				ref = d.joints.middleLeft;
			if (i == right)
				ref = d.joints.middleRight;
			if (i == bottom)
				ref = d.joints.middleBottom;
		...
		{
			if (defaultCornerGraphic != null)
				break;
			if (i != null)
				continue;
			if (i == topRight)
				ref = d.joints.topRight;
			if (i == topLeft)
				ref = d.joints.topLeft;
			if (i == bottomRight)
				ref = d.joints.bottomRight;
			if (i == bottomLeft)
				ref = d.joints.bottomLeft;
			ref.removeChildren();
			ref.addChild(new Bitmap(g));
		}
